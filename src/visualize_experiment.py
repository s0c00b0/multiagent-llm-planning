...
def _compute_room_visit_counts(player_paths: List[List[Tuple[str, int]]], num_players: int) -> List[Counter]:
    ...
    Consecutive duplicates (if bad parse) are collapsed before counting.
    """
...
def visualize_room_visit_heatmap(
    ...
    # Draw the base map (no paths) to reuse layout + room box dimensions.
    ...
    # Normalize counts so 1 visit maps to light color, max_visits to darkest.
    vmax = max(1, max_visits)
    norm = plt.Normalize(vmin=1, vmax=vmax)

    # Overlay heat on visited rooms only.
    for room_name, count in visit_counts.items():
        ...
        # Draw numeric visit count directly on the room (instead of a colorbar).
        # Place it in the top-left of the room box to avoid the room name/icons.
...
def main():
    ...
    try:
        ...
        # Per-player room visit heatmaps
